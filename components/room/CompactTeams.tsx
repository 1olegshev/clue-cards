                    ...
                    {guessers.length === 0 ? (
                      ...
                      guessers.map((p, i) => (
                        <span key={p.id} className={`whitespace-nowrap ${
                          ...
                          {p.avatar} {p.name}{p.id === currentPlayerId ? " (you)" : ""}{i < guessers.length - 1 ? "," : ""}
                        </span>
                      ))
                    )}
                  </div>
          ...
          <div className="flex flex-wrap gap-3">
            {spectators.map((p) => {
              ...
              return (
                ...
                >
                  <div className={`font-medium flex items-center gap-1.5 ${isRoomOwner && onAddSpectator ? "mb-2" : ""}`}>
                    <span>{p.avatar}</span>
